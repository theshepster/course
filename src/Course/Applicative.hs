...
(<$>) ::
  ...
  -> f a
  -> f b
function <$> context = pure function <*> context
...
filtration :: List Bool -> List a -> List a
filtration bs as = foldRight (\(bool, a) out -> if bool then a :. out else out) Nil (zip bs as)
-----------------------
-- SUPPORT LIBRARIES --
-----------------------
